...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    hi = np.matmul(X, W1) + b1
    ac = np.maximum(0.0, hi)
    scores = np.matmul(ac, W2) + b2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    dlogits /= batch_size
    grads['W2'] = np.matmul(ac.T, dlogits) + 2 * reg * W2
    grads['b2'] = dlogits.sum(axis=0)
    dhi = np.matmul(dlogits, W2.T)
    dhi[hi < 0] = 0
    grads['W1'] = np.matmul(X.T, dhi) + 2 * reg * W1
    grads['b1'] = dhi.sum(axis=0)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      idx_batch = np.random.choice(num_train, size=batch_size, replace=True)
      X_batch = X[idx_batch]
      y_batch = y[idx_batch]
      #########################################################################
      #                             END OF YOUR CODE                          #
